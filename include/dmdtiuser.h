...
#ifdef unix
#	define SIGDECL(s)	void (*s) ();
#	define SIGHOLD(s)	s = sigset(SIGPOLL, SIG_HOLD)
#	define SIGREL(s)	sigset (SIGPOLL, SIG_HOLD)
#else
#	define SIGDECL(s)
#	define SIGHOLD(s)
#	define SIGREL(s)
...
/* The system error indicator, errno, is defined by the
 * following macro.  The macro references the errno field in
 * the Proc structure of the current process.
 */
#define errno		(*(int *)((char *)P + 160))

/* The TLI library error indicator, t_errno, is defined by the
 * following macro.  The macro references the t_errno field in
 * the Proc structure of the current process. 
 */
#define t_errno         (*(int *)((char *)P + 162))
