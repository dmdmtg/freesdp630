...
static char _2Vsccsid[]="@(#)CATALOG.c	1.1.1.1	(5/10/87)";
...
{
   request(KBD|MOUSE);
   rectf(&display,Drect,F_CLR); /* bgs changed CLR to OR for GonB */
   Pos.x = 16; Pos.y = 32;
   NamePoint.x = 16; NamePoint.y = 12;
...
#undef outline
#define outline(b,r)	box(b, Rpt(r.origin,add(r.corner, Pt(1,1))), F_XOR)
#ifdef LATER
outline(b,r)
Bitmap *b;
Rectangle  r;
...
ShowName()
{
   ...
   if (ThisIcon != LastIcon) {
     ...
     outline(&display,inset(frame,-2)); outline(&display,inset(frame,-3));
     frame = IconFrame(ThisIcon);
     outline(&display,inset(frame,-2)); outline(&display,inset(frame,-3));
     if (LastIcon == NoIcon) {cursinhibit(); CursPresent=false;}
     if (ThisIcon == NoIcon) {cursallow(); CursPresent=true;}
     LastIcon = ThisIcon;
   }
}
...
NextPage()
{
   ...
   IconList = 0; LastIcon = NoIcon;
   cursallow(); CursPresent = true;
   Pos.x = 16; Pos.y = 32;
}
...
PrintIcon()
{
   ...
   if (((Drect.origin.y+Pos.y) > (Drect.corner.y-(ICONSIZE+16))) && (ArgCount>2)) {
     for (;;) {
       wait(KBD|MOUSE);
