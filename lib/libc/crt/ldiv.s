#ident	"@(#)libc-mc68:crt/ldiv.s	2.7"
...
get_arg2:
	...
	def	lrem%%; val lrem%%; scl 2; type 045; endef
...
lrem2:
	...
	rts
	def	lrem%%; val ~; scl -1; endef

	def	ulrem%%; val ulrem%%; scl 2; type 045; endef
ulrem%%:				# sign of remainder depends only on dividend
	mov.l	%d2,%a0		# save d2 in a0
	mov.l	4(%sp),%d2	# pick up divisor
	bsr.b	uldiv0		# do unsigned division
	mov.l	%d1,%d0		# result is remainder
	rts
	def	ulrem%%; val ~; scl -1; endef

	def	ldiv%%; val ldiv%%; scl 2; type 045; endef
...
ldiv1:
	rts
	def	ldiv%%; val ~; scl -1; endef

	def	uldiv%%; val uldiv%%; scl 2; type 045; endef
uldiv%%:
	mov.l	%d2,%a0		# save d2 in a0
	mov.l	4(%sp),%d2	# pick up divisor
uldiv0:
