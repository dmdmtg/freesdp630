...
static char _2Vsccsid[]="@(#)doprnt.c	1.1.1.5	(6/18/87)";

/*
 *	_Doprnt: common code for Printf, Fprintf, Sprintf, Lprintf, Bprintf
 */
...
/* This is called to send out every character */
...
/* This is called when padding before or after a string with spaces or 0's */
...
/* It is always called for lprintf and bprintf */
static
...
{
	if (iop->file != _NFILE) { /* printf or fprintf call */
		iop->count -= (*ptrptr - (unsigned char *)iop->cp);
		iop->cp = (char *)*ptrptr;
		fflush(iop);
		(void) fwrite(p, 1, n, iop);
		*ptrptr = (unsigned char *)iop->cp;
	} else if(iop->base)	/* lprintf or bprintf call */
		((void (*)())(iop->base))(p, n);
	else /* sprintf call */
		*ptrptr = (unsigned char *) memcpy((char *) *ptrptr, p, n) + n;
}

/* Doprnt does all the work for the Printf routines. It sends the output
 * to the given FILE *iop. If iop->file = _NFILE, this is a dummy file
 * used by sprintf, lprintf, and bprintf. Then if iop->base, this is a
 * pointer to a function which takes a character pointer and a count of
 * characters to be displayed for lprintf or bprintf. Otherwise this is
 * sprintf and it is assumed its size is big enough to hold all the characters
 * given.
 *
 * If iop->file < _NFILE, this is a real file and fflush and fwrite are
 * used. Doprnt will write into the FILE buffer and update iop->count
 * and iop->cp.
 */

int
_Doprnt(iop, format, args)
register FILE	*iop;
...
char *	args;
{
	...
	/* dummy file descriptors (iop->file = _NFILE), bufferend */
	/* may be meaningless.				*/

	unsigned char *bufptr, *bufferend;

	/* This variable counts output characters for return value. */
	int	count = 0;
	...
	/* make sure _iob has been initialized */

	/* initialize buffer pointer and buffer end pointer */
	{ register int fno = iop->file;
	bufptr = (unsigned char *)iop->cp;
	if(fno == _NFILE)
		if(iop->base) /* lprintf or bprintf */
			bufferend = 0; /* force call to _dowrite */
		else /* sprintf */
			bufferend = (unsigned char *)0x7fffffff;
	else /* printf or fprintf */
		bufferend = (unsigned char *)iop->base + BUFSIZ;
	}
	...
	for ( ; ; ) {
		...
		if (fcode == '\0') {  /* end of format; return */
			register int nn = bufptr - (unsigned char *)iop->cp;
			iop->count -= nn;
			iop->cp = (char *)bufptr;
			/* JRG - maybe I should flush the output? nah. */
