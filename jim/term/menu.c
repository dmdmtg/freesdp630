...
static char _2Vsccsid[]="@(#)menu.c	1.1.1.9	(5/8/87)";
...
delmenu(f)
	register f;
{
	register mm=menupos[f];
	register char *x;
	register i;
	/* don't free it; save needless alloc() thrashing (see insmenu) */
	x=name1[mm];		/* save the allocated string... */
	for(i=mm; i<nnames-1; i++)
		name1[i]=name1[i+1];
	name1[--nnames]=x;	/* ... and put it at the end */
	...
	register char *s;
{
	...
	register char **p;
	register char *x;
	for(mm=1; mm<nnames && strcmp(name1[mm]+3, s)<0; mm++)	/* +3 skips '*  */
		;
	x=name1[nnames];		/* a previously allocated string (maybe) */
	for(i=nnames-1; i>=mm; i--)
		name1[i+1]=name1[i];
	for(i=1; i<MAXFILES; i++)
		if(menupos[i]>=mm)
			++menupos[i];
	...
	register char *s;
{
	...
	if((exists=menupos[f])){	/* assignment = */
		movstring(3, name1[menupos[f]], buf);
		delmenu(f);
	}else
		movstring(3, "   ", buf);	
	insmenu(f, s);
	movstring(3, buf, name1[menupos[f]]);
}
inittags(f)
{
	if(f)
		movstring(3, "   ", name1[menupos[f]]);
}
modified(t, mod)
	register Text *t;
{
...
gnamegen(i){
	...
	if(i>=nnames+WRITE)
		return 0;
	return name1[i-WRITE];	/* DIAG is not in menu */
}
/*ARGSUSED*/
...
/* Whichrect:  If the mouse points to the scroll bar on the side of the frame,
 * return SCROLL BAR, else, its just a FRAMERECT. 
 * on t=0, or a new window, return FRAMERECT */

whichrect(t, pt)
	register Text *t;
	Point pt;
{
	if ( t!=0 && ptinrect(pt, t->frame->scrollrect))
		return SCROLLBAR;
	else
		return FRAMERECT;
}
	...
	register but;
{
	...
	if(t && t==worktext && ptinrect(pt, t->frame->scrollrect)){
		if(current!=t)
			curse(t->frame,CURSOROFF);
		(*(but==B2? seek : move))(t, pt, but);
		if(current!=t)
			curse(t->frame,CURSORON);
...
PaintMenu:
	...
	for(p=tro, i=top; i < min(top+lines, items); ++i){
		...
		*to = '\0';
		...
		string(&nediumfont, fill, &screen, q, F_XOR);
		p.y += SPACING;
	}
	savep = mouse.xy;
...
newgetrect(){
	...
	while (!ptinrect(mouse.xy,Drect)) wait(CPU);
/* changed C to c in line below */
	oldcurs = cursswitch(&boxcurs);
	bttns(UP);
	bttns(DOWN);
	if(!bttn3()){
	...
	box(&physical,r,F_XOR);	/* undraw for the last time */
    Return:
/* changed C to c in line below */
	cursswitch(oldcurs);
	return r;
